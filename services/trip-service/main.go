...
func main() {
	...
	// Create listener
	tripServiceAddr := fmt.Sprintf(":%d", config.TripService.Port)
	lis, err := net.Listen("tcp", tripServiceAddr)
	if err != nil {
		slog.Error("failed to listen", "error", err, "address", tripServiceAddr)
		os.Exit(1)
	}
